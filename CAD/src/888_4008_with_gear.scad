...
use <lib/PolyGear/PolyGear.scad>
            ...
            difference() {
                union(){
                    difference()
                    {
                            ...
                            tooth_h=rpm_sensor_h/2-1;
                            translate([0,0,starter_top_h+top_thickness+tooth_h/2])
                            { 
                                difference(){
                                spur_gear(n=22,m=2, w=tooth_h, helix_angle=0);
                                //zkosení pro tisk
                                    translate([0,0,-tooth_h/2-0.1])
                                        difference()
                                        {
                                            cylinder(r=starter_rope_d/2+5, h=3);
                                            cylinder(r1=starter_rope_d/2, r2=starter_rope_d/2+5, h=3);                                
                                        }
                                    translate([0,0,tooth_h/2+0.1])
                                        for(i=[0:22])
                                        {
                                            rotate([0,0,i*360/22])
                                            translate([starter_rope_d/2-2,0,0])
                                            {
                                                translate([0,0.25,0])
                                                    rotate([-45,0,0])
                                                        cube([8,3,2]);
