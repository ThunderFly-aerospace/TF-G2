...
module 888_1010(){
            ...
            union(){
                ...
                for(m = [0, 1]) mirror([0,m,0]) difference(){
                    union(){
                        ...
                        // Bocni svisle placky - spodni cast
                        hull(){
                            // Pro pripevneni adapteru na pylon
                            translate([-25, airspeed_width/2-side_wall, rotor_head_pylon_adapter_z - M3_screw_diameter*1.5]) cube([30, side_wall, 1]);
                            translate([-25, airspeed_width/2-side_wall, -20]) cube([30, side_wall, 15]);                
                        }
                    ...
                    translate([-pylon_adapter_mount_holes_distance, 0, rotor_head_pylon_adapter_z]) rotate([-90, 0, 0]) cylinder(d = M3_screw_diameter, h = 100, $fn=20);
                    translate([0, airspeed_width/2, rotor_head_pylon_adapter_z]) rotate([-90, 0, 0]) cylinder(d = M3_nut_diameter, h = 100, $fn=20);
                    translate([-pylon_adapter_mount_holes_distance, airspeed_width/2, rotor_head_pylon_adapter_z]) rotate([-90, 0, 0]) cylinder(d = M3_nut_diameter, h = 100, $fn=20);
                }
    ...
    // objekty pro sestavu
    translate([0, 0, pitch_axis_z]) rotate([90, 0, 90]) rotate([-rotor_head_pitch, 0, 0]) children(0) children(1);
